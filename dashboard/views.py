...
def ManageNotifications(request):
    sends_notifications = SendNotifications.objects.all().order_by('-id')
    sender = request.user
    countrys = CountrysModel.objects.all()
    if request.method == 'POST':
        title = request.POST.get('title')
        msg = request.POST.get('msg')
        send_local = (request.POST.get('send_local', False) == 'on')
...
def DeleteNotifications(request, id):
    send_notification = SendNotifications.objects.get(id=id)
